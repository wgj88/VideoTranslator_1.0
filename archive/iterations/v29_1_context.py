...
def run_v29_precision():
    print(f"\n[V29.1-Context] 正在执行第二轮【上下文锚定】手术...")
    ...
    # 锁定 Line 11 和 Line 12
    prev_text = data[11]['zh'] # 这产品纯属忽悠...
    target_text = data[12]['zh'] # 赶紧坐下吧！
    
    combined_text = prev_text + "。" + target_text
    print(f"  -> [步骤 1] 正在生成 8s 级联合稳态波形: '{combined_text}'")
    
    seed = role_lib['SPEAKER_00']
    wav = db.model.generate(text=combined_text, prompt_wav_path=seed['wav'], prompt_text=seed['text'])
    raw_p = os.path.join(temp_dir, "v29_1_combined.wav")
    sf.write(raw_p, wav, db.sample_rate)
    
    # 2. AI 审计
    print(f"  -> [步骤 2] AI 正在长音频中精确定位“赶紧”...")
    ...
    if res['segments']:
        ...
        for w in all_words:
            # 由于可能识别为“赶紧”、“赶快”或“坐下”
            if any(k in w['word'] for k in ["赶", "紧", "坐", "下"]):
                start_cut = w['start'] - 0.15 # 留出 150ms 的自然气口
                found = True
                break
    ...
    if not found:
        print("  ⚠️ 未能通过关键词定位，将回退到物理比例切割...")
        start_cut = (len(prev_text) / len(combined_text)) * (len(wav)/db.sample_rate)

    print(f"     📍 手术切割点锁定: {start_cut:.3f}s -> {end_cut:.3f}s")
    
    # 3. 物理切片提取
    output_wav = r"E:\VideoTranslator_Project\output_final\V29_1_RESCUED_SIT_DOWN.wav"
    ...
    print(f"\n🏆 救治成功！请听听这个稳态版的“赶紧坐下吧”：{output_wav}")
